...
namespace ForexApp.Services
{
    public class ForexService : IForexService
    {
        ...
        public async Task<IEnumerable<QuoteDto>> GetQuotes(string baseCurrency, string[] targetCurrencies)
        {
            // Send API request
            var apiKey = this.forexServiceConfiguration.ApiKey;
            var currencies = HttpUtility.UrlEncode(string.Join(",", targetCurrencies));

            var uri = $"https://api.freecurrencyapi.com/v1/latest?apikey={apiKey}&base_currency={baseCurrency}&currencies={currencies}";
            ...
            var quoteDtos = latestQuotesDto.Data
                .Select(c => new QuoteDto { Symbol = c.Key, Price = c.Value })
                .ToArray();
